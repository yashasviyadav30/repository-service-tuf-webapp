...
def resolve_trusted_root(value: str, timeout: float = 10.0) -> bytes:
    ...
    if text.startswith(("http://", "https://")):
        ...
        return _fetch_anchor(text, timeout)

    return decoded


def _fetch_anchor(url: str, timeout: float) -> bytes:
    """Fetch the trust anchor, stopping if it grows past the cap.

    UpdaterConfig bounds every other fetch. This one is written here, so it
    is bounded here. Reading the whole response and then measuring it is not
    a bound, because by then the host has decided how much to hold.
    """
    received = bytearray()
    too_large = MetadataUnavailableError(
        f"the trust anchor at {url} is larger than {ROOT_MAX_BYTES} bytes"
    )
    ...
    try:
        with httpx.stream("GET", url, timeout=timeout) as response:
            ...
            for chunk in response.iter_bytes():
                received += chunk
                if len(received) > ROOT_MAX_BYTES:
                    raise too_large
    except httpx.HTTPError as exc:
        raise MetadataUnavailableError(
            f"could not fetch the trust anchor from {url}"
        ) from exc
