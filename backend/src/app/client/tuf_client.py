...
def _fetch_anchor(url: str, timeout: float) -> bytes:
    ...
    Redirects are followed, as python-tuf's own fetcher does, because
    storage is commonly served from behind one. The body is counted as it
    arrives, so the cap holds whatever the final response turns out to be.
    """
    received = bytearray()
