...
from app.client.error import MetadataUnavailableError, UnknownRoleError
...
from app.models.views import DelegatedView, RepositoryView, RoleView
...
def _read_trust_directory(
    trust_dir: Path,
) -> tuple[list[RoleView], dict[str, Metadata], dict[str, bytes]]:
    """Describe every role this pass verified, and keep the bytes it read.

    The bytes are kept because the directory does not outlive the pass and a
    reader may ask to see the document afterwards. Re-serialising the parsed
    copy would answer with bytes nobody signed: the same content, laid out
    the way this library writes JSON rather than the way the repository did,
    and a signature is over the layout as much as the content.
    """
...
async def load_repository(
    ...
    try:
        ...
        roles, documents, raw = _read_trust_directory(trust_dir)
    finally:
...
def _fetch_delegated(
    ...
    try:
        ...
        # Bytes that will not parse are not metadata. A repository can serve
        # anything under this name, and a role nobody can read is a role with
        # nothing to show, not a fault in this service.
        raise MetadataUnavailableError(
            f"{url} could not be read as metadata: {exc}"
        ) from exc
