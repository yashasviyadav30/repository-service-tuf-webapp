...
class MetadataService:
    ...
    @staticmethod
    def _is_top_level(view: RepositoryView, name: str) -> bool:
        return any(role.name == name for role in view.roles)

    async def get_role(self, name: str) -> RoleDetailResponse:
        """Describe one role, fetching it only if it is delegated."""
        view = await self.get_repository_view()

        if self._is_top_level(view, name):
            return presenters.build_role_detail(view, name)

        loaded = await self._delegated(view, name)
        return presenters.build_delegated_detail(view, loaded)

    async def get_raw_metadata(self, name: str) -> bytes:
        """Return the bytes one role was verified from.

        A top-level role is answered from the reading that verified it, which
        kept what it read. A delegated role is fetched and checked now, and
        the bytes that passed that check are the ones returned.
        """
        view = await self.get_repository_view()

        if self._is_top_level(view, name):
            document = view.raw.get(name)
            if document is None:
                raise MetadataUnavailableError(
                    f"the verified copy of {name} was not kept"
                )
            return document

        return (await self._delegated(view, name)).raw
    ...
    async def _delegated_or_none(
        self,
        view: RepositoryView,
        limiter: asyncio.Semaphore,
        name: str,
    ) -> tuple[str, DelegatedView | None]:
        """Load one delegated role, or report that it could not be read."""
        async with limiter:
            try:
    ...
    async def _gather_artifacts(
        ...
        results = await asyncio.gather(
            *(
                self._delegated_or_none(view, limiter, name)
                for name in sorted(view.delegated)
            )
        )
    ...
    async def get_artifact_bins(self) -> ArtifactBinsResponse:
    ...
    async def get_artifacts(
        ...
        if role:
            ...
            # Narrowing to one role narrows what is missing from it too:
            # reporting every unreachable bin here would name bins the
            # caller did not ask about.
            unavailable = [name for name in unavailable if name == role]
        if search:
            needle = search.lower()
