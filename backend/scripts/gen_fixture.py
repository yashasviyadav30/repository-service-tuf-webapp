...
Creates four variants under ``fixtures/``:

    good/         every role valid and in date
    expired/      same keys, but timestamp already past its expiry
    rolled-back/  a bin correctly signed, but older than snapshot pins
...
# Named after real packages so the artifact list reads like an index rather
# than a placeholder. Eight packages at four versions fills each bin with 32,
# which is more than one page holds.
PACKAGES = (
    "certifi",
    "charset-normalizer",
    "click",
...
def _artifacts(bin_name: str, offset: int) -> list[tuple[str, TargetFile]]:
    """List the artifacts one bin vouches for.

    Enough of them that a reader has to page through the list rather than
    seeing every one, which is the case a two-entry repository never reaches.
    The files themselves do not exist: this repository is about metadata, and
    the metadata is what is signed.
    """
    listed: list[tuple[str, TargetFile]] = []
    for package in PACKAGES:
        for minor in range(VERSIONS_PER_PACKAGE):
            path = f"{bin_name}/{package}-1.{minor + offset}.0.tar.gz"
...
def _build(
    out_dir: Path,
    timestamp_expiry_days: int,
    rolled_back_bin: str | None = None,
) -> None:
    ...
    for offset, name in enumerate(BINS):
        ...
        # Each bin carries its own version range, so a search matches one of
        # them and not the same package in every bin.
        for path, target in _artifacts(name, offset * VERSIONS_PER_PACKAGE):
    ...
    # One bin is pinned a version ahead of the file published for it, and
    # that file is correctly signed. A reader checking only the signature
    # accepts an artifact list the repository has already moved on from.
    if rolled_back_bin is not None:
        snapshot.meta[f"{rolled_back_bin}.json"] = MetaFile(version=2)
    ...
    # The version snapshot now points at, holding what version 1 said.
    if rolled_back_bin is not None:
        shutil.copy(
            out_dir / f"1.{rolled_back_bin}.json",
            out_dir / f"2.{rolled_back_bin}.json",
        )
...
def main() -> None:
    ...
    _build(
        ...
        rolled_back_bin=BINS[0],
    )
    _tamper(FIXTURES / "good", FIXTURES / "tampered")

    for variant in ("good", "expired", "rolled-back", "tampered"):
