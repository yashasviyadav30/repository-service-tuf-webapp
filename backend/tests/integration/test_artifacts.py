...
class TestABinThatCouldNotBeRead(RepositoryTestCase):
    ...
    def test_narrowing_to_a_role_narrows_what_is_missing_from_it(
        self, webapp, monkeypatch
    ) -> None:
        """Asking about one bin should not report another bin's trouble."""
        original = service_module.load_delegated_role
        ...
        asked_about_the_healthy_bin = client.get(
            "/api/v1/artifacts?role=bins-0"
        ).json()
        asked_about_the_broken_bin = client.get(
            "/api/v1/artifacts?role=bins-1"
        ).json()

        assert asked_about_the_healthy_bin["unavailable"] == []
        assert asked_about_the_broken_bin["unavailable"] == ["bins-1"]
