...
from app.enums import ExpiryBand
...
class TestExpiryBand(UnitTestCase):
    ...
    def test_a_role_lapsed_under_an_hour_counts_minutes(self) -> None:
        """Whole hours read as "expired 0h ago" for the first sixty minutes
        after a role lapses, which is when a reader most needs the number."""
        band, phrase = expiry_band(at(minutes=-20), NOW)

        assert band is ExpiryBand.EXPIRED
        assert phrase == "expired 20 minutes ago"

    def test_one_minute_is_not_pluralised(self) -> None:
        _, phrase = expiry_band(at(minutes=-1), NOW)

        assert phrase == "expired 1 minute ago"
